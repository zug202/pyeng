...
def func_dev_search(client_name):
    ...
    while True:
        ...
        dev_for_search = input(
            f"\nВведите '1' чтобы вывести весь список устройств для {client_name}\nИли введите hostname устройств(а) для поиска (пример: sko-data-ss%int): ")
        try:
            if int(dev_for_search) == 1:
                for device in m.np.device_details.get(cpyKey=DICT_OF_CLIENTS[client_name]):
                ...
                if device.configTime:
                    ...
                    output_false.append(
                        [str(device.deviceSysname), str(device.deviceIp), str(device.productId), str(device.swType),
                         str(device.swVersion), str(device.configTime).replace("T", " "), str(device.deviceId)])
        if len(output_false):
            print("\nУстройства, с которых не получена конфигурация:\n",
        ...
        commit = input("\nПродолжить работу с данными устройствами (yes/no)? [yes] ")
        try:
...
def func_search_in_runnconf(client_name, devices_selection):
    ...
    for runn_conf in m.np.config.get(cpyKey=DICT_OF_CLIENTS[client_name], configType="STANDARD RUNNING"):
...
def func_running_config(client_name, devices_selection):
    ...
    for hostname, dev_id in dict_of_dev.items():
        for runn_conf in m.np.config.get(cpyKey=DICT_OF_CLIENTS[client_name], deviceId=dev_id,
...
def func_show_command(client_name, devices_selection):
    ...
    list_show_commands = []
    for cli_avail in m.np.cli_available.get(cpyKey=DICT_OF_CLIENTS[client_name], deviceId=devices_selection[0][6]):
    ...
    for device in devices_selection:
        ...
        device_id = device[-1]
        for command in dict_selected_show_commands.values():
            for show_data in m.np.cli.get(cpyKey=DICT_OF_CLIENTS[client_name], deviceId=device_id, command=command):
